...
function appendChatMessage(message) {
    ...
    time.textContent = new Date(message.timestamp).toLocaleTimeString('ru-RU', {
        hour: '2-digit',
        minute: '2-digit'
    });
    ...
    copyBtn.className = 'chat-btn-copy';
    ...
    reactBtn.className = 'chat-btn-react';
    ...
    reactBtn.addEventListener('click', async () => {
        ...
        reactionSpan.className = 'chat-reaction';
    ...
    if (message.reactions && message.reactions.poop) {
        const reactionSpan = document.createElement('span');
        reactionSpan.className = 'chat-reaction';
        reactionSpan.textContent = '💩'.repeat(message.reactions.poop);
        text.appendChild(reactionSpan);
        reactBtn.classList.add('reacted');
    }
