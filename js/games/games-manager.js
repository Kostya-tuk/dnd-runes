    ...
    for (const [key, game] of Object.entries(GAMES)) {
        ...
        btn.addEventListener('click', ()=>{
            if (availableGames[key]) {
                // No need to pass code for permanent unlock; game already available
                activeGameCode = null; // no code needed if already unlocked
                openGameModal(key);
            }
        });
...
function renderCodeInput(container) {
    ...
    btn.addEventListener('click', async ()=>{
        ...
        // Permanently unlock for this player
        await DB.activateCodeForPlayer(result.id, result.game_type, DB.currentUser.id);
        DB.availableGames[result.game_type] = true;
        showToast(`Игра "${GAMES[result.game_type].name}" разблокирована навсегда!`);
        renderPanels();
    });
