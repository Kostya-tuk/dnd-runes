const DB = {
        ...
        if (!isMaster) {
            ...
            if (!existing) {
                await dbClient.from('player_progress').insert({
                    room_id: roomId, player_id: this.currentUser.id, player_name: nickname,
                    rune_guesses: JSON.stringify({}), unlocked_runes: JSON.stringify([]),
                    games_completed: JSON.stringify([]), unlocked_games: JSON.stringify([])
                });
            } else {
                // Load unlocked games into availableGames
                if (existing.unlocked_games) {
                    const arr = JSON.parse(existing.unlocked_games);
                    arr.forEach(g => this.availableGames[g] = true);
                }
            }
        }
    ...
    async getPlayersProgress() {
        ...
        return (data||[]).map(p => ({ player_id: p.player_id, player_name: p.player_name, guesses: typeof p.rune_guesses==='string'?JSON.parse(p.rune_guesses):{}, unlocked: typeof p.unlocked_runes==='string'?JSON.parse(p.unlocked_runes):[], completed: typeof p.games_completed==='string'?JSON.parse(p.games_completed):[], unlocked_games: p.unlocked_games ? (typeof p.unlocked_games==='string'?JSON.parse(p.unlocked_games):p.unlocked_games) : [] }));
    },
    ...
    async activateCodeForPlayer(codeId, gameType, playerId) {
        // Mark code as used permanently, and unlock the game for this player
        await dbClient.from('game_codes').update({ used: true, active: false }).eq('id', codeId);
        const { data: progress } = await dbClient.from('player_progress').select('unlocked_games').eq('room_id', this.currentRoom.id).eq('player_id', playerId).single();
        let games = [];
        if (progress && progress.unlocked_games) {
            games = typeof progress.unlocked_games === 'string' ? JSON.parse(progress.unlocked_games) : progress.unlocked_games;
        }
        if (!games.includes(gameType)) games.push(gameType);
        await dbClient.from('player_progress').update({ unlocked_games: JSON.stringify(games) }).eq('room_id', this.currentRoom.id).eq('player_id', playerId);
        // Update local availableGames if it's the current player
        if (this.currentUser && this.currentUser.id === playerId) {
            this.availableGames[gameType] = true;
        }
    },

    async loadUnlockedGames(playerId) {
        const { data: progress } = await dbClient.from('player_progress').select('unlocked_games').eq('room_id', this.currentRoom.id).eq('player_id', playerId).single();
        if (progress && progress.unlocked_games) {
            const games = typeof progress.unlocked_games === 'string' ? JSON.parse(progress.unlocked_games) : progress.unlocked_games;
            games.forEach(g => this.availableGames[g] = true);
        }
    },
